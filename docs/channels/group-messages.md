...
- Session store entries appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.openclaw/state/openclaw.sqlite` by default; legacy/custom JSON stores export as `sessions.json`); a missing entry just means the group hasn't triggered a run yet.
