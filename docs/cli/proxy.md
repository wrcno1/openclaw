...
- Captures default to the shared state database
  (`~/.openclaw/state/openclaw.sqlite`); use
  `OPENCLAW_DEBUG_PROXY_DB_PATH` only for an explicit one-off capture database.
